...
os.mkdir('dataminingcontest/borrower_credit/M2/featurescore')
os.mkdir('dataminingcontest/borrower_credit/M2/model')
os.mkdir('dataminingcontest/borrower_credit/M2/preds')



#离散化特征的计数特征
test_nd = pd.read_csv('dataset/borrower_credit/test_x_nd.csv')[['uid','n1','n2','n3','n4','n5','n6','n7','n8','n9','n10']]
train_nd = pd.read_csv('dataset/borrower_credit/train_x_nd.csv')[['uid','n1','n2','n3','n4','n5','n6','n7','n8','n9','n10']]
trainunlabeled_nd = pd.read_csv('dataset/borrower_credit/train_unlabeled_nd.csv')[['uid','n1','n2','n3','n4','n5','n6','n7','n8','n9','n10']]

#缺失值个数的离散化特征
test_dnull = pd.read_csv('dataset/borrower_credit/test_x_null.csv')[['uid','discret_null']]
train_dnull = pd.read_csv('dataset/borrower_credit/train_x_null.csv')[['uid','discret_null']]
trainunlabeled_dnull = pd.read_csv('dataset/borrower_credit/train_unlabeled_null.csv')[['uid','discret_null']]
...
discret_train = pd.read_csv("dataset/borrower_credit/train_x_discretization.csv")[['uid']+fs]
discret_test = pd.read_csv("dataset/borrower_credit/test_x_discretization.csv")[['uid']+fs]
discret_train_unlabeled = pd.read_csv("dataset/borrower_credit/train_unlabeled_discretization.csv")[['uid']+fs]

#排序特征
rank_feature_score = pd.read_csv('./rank_feature_score.csv')
fs = list(rank_feature_score.feature[0:500])
rank_train_x = pd.read_csv("dataset/borrower_credit/train_x_rank.csv")
rank_train = rank_train_x[fs] / float(len(rank_train_x))
rank_train['uid'] = rank_train_x.uid

rank_test_x = pd.read_csv("dataset/borrower_credit/test_x_rank.csv")
rank_test = rank_test_x[fs] / float(len(rank_test_x))
rank_test['uid'] = rank_test_x.uid

rank_train_unlabeled_x = pd.read_csv("dataset/borrower_credit/train_unlabeled_rank.csv")
...
raw_train_x = pd.read_csv("dataset/borrower_credit/train_x.csv")[['uid']+fs]
raw_train_y = pd.read_csv("dataset/borrower_credit/train_y.csv")
raw_train = pd.merge(raw_train_x,raw_train_y,on='uid')
del raw_train_x,raw_train_y

raw_test = pd.read_csv("dataset/borrower_credit/test_x.csv")[['uid']+fs]
raw_train_unlabel = pd.read_csv('dataset/borrower_credit/train_unlabeled.csv')[['uid']+fs]
...
xgb717_predict_unlabeled_data = pd.read_csv('dataminingcontest/borrower_credit/M2/xgb717_predict_unlabeled_data.csv')
...
def pipeline(iteration,random_seed,feature_num,rank_feature_num,discret_feature_num,gamma,max_depth,lambd,subsample,colsample_bytree,min_child_weight):
    ...
    model.save_model('dataminingcontest/borrower_credit/M2/model/xgb{0}.model'.format(iteration))
    
    #predict test set
    test_y = model.predict(dtest)
    test_result = pd.DataFrame(test_uid,columns=["uid"])
    test_result["score"] = test_y
    test_result.to_csv("dataminingcontest/borrower_credit/M2/preds/xgb{0}.csv".format(iteration),index=None,encoding='utf-8')
    ...
    with open('dataminingcontest/borrower_credit/M2/featurescore/feature_score_{0}.csv'.format(iteration),'w') as f:
