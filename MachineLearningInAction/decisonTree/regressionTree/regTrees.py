...
def linearSolve(dataSet):
    """返回线性模型的自变量，因变量和参数值/回归系数
    :param dataSet:
    :return:如果矩阵的逆不存在，就会抛出异常
    """
    m, n = shape(dataSet)
    ### one可以用来构造数组，里面的元素全都用1填充
    X = mat(ones((m, n)))
    Y = mat(ones((m, 1)))
    ## 将X,Y中的数据格式化成自变量X和目标变量Y
    X[:, 1:n] = dataSet[:, 0:n - 1]
    Y = dataSet[:, -1]
    xTx = X.T * X
    ## det是矩阵的行列式
...
def treeForeCast(tree, inData, modelEval=regTreeEval):
    ...
    :param tree:树结构（字典）
    :param inData:测试数据
    :param modelEval:模型返回的叶结点计算方式的引用
    :return:y的预测值
    """
    if (not isTree(tree)):
...
trainMat = mat(loadDataSet('F:\code\Python-Project\dataset\\bikeSpeedVsIq_train.txt'))
testMat = mat(loadDataSet('F:\code\Python-Project\dataset\\bikeSpeedVsIq_test.txt'))
myTree = createTree(trainMat, ops=(1, 20))
yHat = createForeCast(myTree, testMat[:, 0])
## 相关系数，也称R^2值，可以通过corrcoef来求解，越接近于1越相关
